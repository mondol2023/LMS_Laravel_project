...
const loadAudio = () => {
    if (audioRef.value) {
        audioRef.value.src = '/audio/listening9.mp3';
        audioRef.value.load();
    }
};
...
const handleProgressSliderChange = (event: Event) => {
    ...
    <nav class="sticky top-0 z-50 border-b border-gray-200 bg-white/95 shadow-sm backdrop-blur-sm">
        <div class="container mx-auto px-3 sm:px-4 lg:px-6">
            ...
            <div class="block md:hidden">
                ...
                <!-- Audio Progress Bar (Mobile) -->
                <div class="border-t border-gray-200 bg-gray-50">
                    <!-- Main Progress Section -->
                    <div class="flex items-center gap-2 px-2 py-2">
                        <!-- Current Time -->
                        <span class="w-10 flex-shrink-0 font-mono text-xs text-gray-600">{{ formatAudioTime(currentTime) }}</span>
                        ...
                        <div class="relative flex-1">
                            <input
                                ...
                                @input="handleProgressSliderChange"
                                class="progress-slider h-2 w-full cursor-pointer appearance-none rounded-lg bg-gray-200"
                                :style="{ '--progress': `${getProgressPercentage()}%` }"
                            />
                        </div>

                        <!-- Duration -->
                        <span class="w-10 flex-shrink-0 font-mono text-xs text-gray-600">{{ formatAudioTime(duration) }}</span>
                    </div>

                    <!-- Volume Control Row -->
                    <div class="flex items-center justify-center gap-2 px-2 pb-2">
                        <svg class="h-3 w-3 flex-shrink-0 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path
                                stroke-linecap="round"
                                stroke-linejoin="round"
                                stroke-width="2"
                                d="M15.536 8.464a5 5 0 010 7.072M6 10H4a2 2 0 00-2 2v2a2 2 0 002 2h2l4 4V6l-4 4z"
                            ></path>
                        </svg>
                        <input
                            type="range"
                            ...
                            step="0.1"
                            :value="volume"
                            @input="changeVolume(parseFloat($event.target.value))"
                            class="volume-slider h-1 max-w-24 flex-1 cursor-pointer appearance-none rounded-lg bg-gray-200"
                            :style="{ '--volume': `${volume * 100}%` }"
                        />
                        <span class="w-8 text-center text-xs text-gray-500">{{ Math.round(volume * 100) }}%</span>
                    </div>
                </div>
            </div>
