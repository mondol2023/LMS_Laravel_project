...
Meat Production requires relatively more lands than crop production. Some people think as land is becoming scare, the world’s meat consumption should be reduced.

What measures could be taken to reduce world’s meat consumption? What kind of problem such measures caused?

Give reasons for your answer and include any relevant examples from your own knowledge or experience.
Write at least 250 words.`);

// Resize functionality
const PANEL_WIDTH_KEY = 'writing-part2-ielts011-panel-width';
...
watch(answer, (newValue) => {
    ...
    // Auto-save to draft
    if (autoSaver.value) {
        autoSaver.value.save('part2', newValue);
    }
});
...
onMounted(async () => {
    ...
    autoSaver.value = draftService.createAutoSaver(userPhone, examId, 'writing', 'part2');

    // Load saved drafts
    try {
        const draftsResponse = await autoSaver.value.getDrafts();
        if (draftsResponse.success && draftsResponse.data.part2) {
            const savedAnswer = draftsResponse.data.part2.part2;
            if (savedAnswer) {
                answer.value = savedAnswer;
                console.log('Loaded Writing Part 2 draft:', savedAnswer.substring(0, 50) + '...');
            }
        }
    } catch (error) {
        console.error('Failed to load Writing Part 2 draft:', error);
    }
        ...
        <div class="mx-auto p-3 sm:p-4 lg:p-6">
            <div ref="containerRef" class="relative flex flex-col gap-0 lg:flex-row" :class="{ 'select-none': isResizing }">
                ...
                >
                    <textarea
                        v-model="answer"
                        class="h-[40vh] w-full resize-none rounded-lg border-2 border-gray-300 bg-white p-3 text-base text-gray-900 transition-all duration-200 focus:border-purple-500 focus:ring-1 focus:ring-purple-500 focus:outline-none sm:h-[50vh] sm:p-4 sm:text-lg lg:h-[70vh]"
