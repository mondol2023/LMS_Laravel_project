...
// Check if time is critical (2 minutes or less - starts at 2:00)
const isTimeCritical = computed(() => timeRemaining.value <= 120 && timeRemaining.value > 0);

// Check if time has ended
const timeHasEnded = computed(() => timeRemaining.value === 0);
...
const formatTimeDisplay = computed(() => {
    ...
    if (isTimeCritical.value || timeHasEnded.value) {
        if (minutes > 0) {
            return `${minutes}:${paddedSeconds} remaining`;
        }
        return `0:${paddedSeconds} remaining`;
    }
...
const initializeTimer = () => {
    ...
    // Use audio duration if available, otherwise fall back to timeLimit prop
    const totalSeconds = audioDuration.value || props.timeLimit * 60;
...
const loadAudio = () => {
    if (audioRef.value) {
        audioRef.value.src = '/audio/listening16.mp3';
        audioRef.value.load();
    }
};
...
const onTimeUpdate = () => {
    if (audioRef.value) {
        ...
        // Sync timer with audio playback position for perfect accuracy
        if (audioDuration.value && audioRef.value.duration > 0) {
            const remaining = Math.ceil(audioRef.value.duration - audioRef.value.currentTime);
...
const onLoadedMetadata = () => {
    if (audioRef.value) {
        // Sync timer with actual audio duration (in seconds, rounded up)
        audioDuration.value = Math.ceil(audioRef.value.duration);
        // Only update timeRemaining if exam hasn't started yet
        if (!testStartTime.value) {
            timeRemaining.value = audioDuration.value;
        }
    }
};
...
// Watch for examStarted to begin timer
watch(
    () => props.examStarted,
    (started) => {
        if (started) {
            ...
            startTimer();
            requestWakeLock();
            // Audio is played via playAudio() called from parent on user click
        }
    },
);

// Expose playAudio for parent to call on user click (required for browser autoplay policy)
const playAudio = () => {
    if (audioRef.value) {
        audioRef.value.volume = props.audioVolume / 100;
        audioRef.value.play().catch(() => {});
    }
};

defineExpose({ playAudio, audioRef });
...
onMounted(() => {
    ...
    // Only start timer if exam already started (for backwards compatibility)
    // Audio must be played via playAudio() from parent on user click
