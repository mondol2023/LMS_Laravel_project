...
const questionText = ref(`Globalization has led to the spread of similar cultures around the world. Do you think this is a positive or negative development? Give reasons and examples to support your answer.
`);

// Header text segments for highlighting
const headerTextSegments = ref([
    { id: 'header-title', text: 'Part 2', offset: -100 },
    { id: 'header-instructions', text: 'You should spend about 40 minutes on this task. Write at least 250 words.', offset: -50 },
]);
...
const { highlights, selectedText, selectionRange, colors, addHighlight, deleteHighlight, findOverlappingHighlights, applyHighlightsToText } = useHighlight();

// Get highlighted header text by segment id
const getHighlightedHeaderText = (segmentId: string) => {
    const segment = headerTextSegments.value.find((s) => s.id === segmentId);
    if (!segment) return '';
    ...
    for (const annotation of sorted) {
        ...
        const plainEnd = Math.min(segment.text.length, annotation.end - segmentOffset);

        if (plainStart < plainEnd) {
            const before = result.substring(0, plainStart);
            const annotated = result.substring(plainStart, plainEnd);
            const after = result.substring(plainEnd);

            if (annotation.type === 'note') {
                result = `${before}<mark class="note-highlight" data-note-id="${annotation.id}">${annotated}</mark>${after}`;
            } else {
...
// Handle header text selection - supports multiline across segments
const handleHeaderTextSelect = () => {
    setTimeout(() => {
        ...
        if (rect && (rect.width > 0 || rect.height > 0)) {
            ...
            // Helper to get absolute offset for a node position
...
const applyHighlight = (color: string) => {
    ...
    // "Last action wins" - remove any overlapping notes when highlighting
...
const saveNote = () => {
    ...
    // "Last action wins" - remove any overlapping highlights when adding a note
...
<template>
    <div class="min-h-screen overflow-y-auto pb-20 sm:pb-24 md:pb-32">
        ...
        <div class="mx-4 mt-2 part-header-box border-b-1 border-gray-400 bg-gray-100 px-4 py-3 sm:mx-6 lg:mx-8"
             @mouseup="handleHeaderTextSelect" @click="handleContentClick">
            <p class="text-sm font-bold text-gray-900 sm:text-base select-text" data-segment-id="header-title"
               v-html="getHighlightedHeaderText('header-title')"></p>
            <p class="text-sm text-gray-700 select-text" data-segment-id="header-instructions"
               v-html="getHighlightedHeaderText('header-instructions')"></p>
        </div>
            ...
            <div ref="containerRef" class="relative flex flex-col gap-0 lg:flex-row" :class="{ 'select-none': isResizing }">
                ...
                <div class="answer-panel max-h-screen overflow-y-auto bg-white px-4 py-2 sm:px-6" :style="{ '--panel-width': `${leftPanelWidth}%` }">
                    <textarea
                        v-model="answer"
                        :style="textStyle"
                        class="h-[30vh] w-full resize-none border border-gray-400 bg-white p-3 text-black transition-all duration-200 focus:border-black focus:outline-none sm:h-[40vh] sm:p-4 lg:h-[45vh]"
...
/* Enable selection on header box children */
.part-header-box .select-text,
.part-header-box .select-text * {
    user-select: text;
    -webkit-user-select: text;
}

/* Ensure mark elements inside select-text are selectable */
.select-text mark,
.select-text mark * {
    user-select: text;
    -webkit-user-select: text;
