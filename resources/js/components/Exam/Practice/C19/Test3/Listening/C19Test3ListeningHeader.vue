...
interface Props {
    ...
    isTimeUp?: boolean;
    part1Notes?: Array<{ id: number; text: string; selectedText: string; note: string; start: number; end: number; part?: string }>;
    part2Notes?: Array<{ id: number; text: string; selectedText: string; note: string; start: number; end: number; part?: string }>;
    part3Notes?: Array<{ id: number; text: string; selectedText: string; note: string; start: number; end: number; part?: string }>;
    part4Notes?: Array<{ id: number; text: string; selectedText: string; note: string; start: number; end: number; part?: string }>;
}
...
const loadAudio = () => {
    if (audioRef.value) {
        audioRef.value.src = '/audio/listening17.mp3';
        audioRef.value.load();
    }
};
        ...
        <div class="container mx-auto px-3 sm:px-4 lg:px-6">
            ...
            <div class="block md:hidden">
                ...
                <div class="flex h-14 items-center justify-between">
                    <div class="flex items-center gap-2">
                        <div class="relative">
                            <div
                                class="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-rose-500 to-pink-600 shadow-lg shadow-rose-500/30"
                            >
                        ...
                        <h1 class="bg-gradient-to-r from-gray-900 to-gray-600 bg-clip-text text-sm font-bold text-transparent">C19 Test 3</h1>
                    </div>
                    <button
                        @click="handleSubmit"
                        class="group flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-rose-500 to-pink-600 px-3 py-1.5 text-xs font-semibold text-white shadow-lg shadow-rose-500/30 transition-all duration-300 hover:scale-105 hover:shadow-xl active:scale-95"
                    >
                ...
                <div class="mb-3 rounded-2xl border border-gray-200 bg-white p-3 shadow-sm">
                    <div class="flex items-center gap-3">
                        <button
                            @click="skipBackward"
                            class="flex h-8 w-8 items-center justify-center rounded-full text-gray-500 transition-all hover:bg-gray-100 hover:text-rose-600 active:scale-95"
                        >
                            <SkipBack class="h-4 w-4" />
                        </button>
                        <button
                            @click="togglePlayPause"
                            class="group relative flex h-11 w-11 items-center justify-center rounded-full bg-rose-600 text-white shadow-md transition-all duration-200 hover:bg-rose-700 hover:shadow-lg active:scale-95"
                        >
                        ...
                        <div class="ml-auto font-mono text-xs">
                            <span class="font-medium text-rose-600">{{ formatTime(currentTime) }}</span>
                    ...
                    <div class="group mt-3">
                        <div class="relative h-1.5 cursor-pointer overflow-hidden rounded-full bg-gray-200" @click="seekByClick">
                            <div
                                class="absolute inset-y-0 left-0 rounded-full bg-rose-500 transition-all duration-100"
                                :style="{ width: progressPercent + '%' }"
                            ></div>
                            <div
                                class="absolute top-1/2 h-3.5 w-3.5 -translate-y-1/2 rounded-full bg-rose-600 opacity-0 shadow-md transition-all group-hover:opacity-100"
                    ...
                    <div class="mt-3 flex items-center gap-2">
                        <button @click="toggleMute" class="text-gray-500 transition-all hover:text-rose-600">
                ...
                <div class="flex items-center justify-between pb-3">
                    ...
                    >
                        ...
                        <span
                            ...
                            class="absolute -top-1.5 -right-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-gradient-to-br from-rose-500 to-pink-600 text-[10px] font-bold text-white shadow-lg"
                        >
                            {{ totalNotesCount }}
                        </span>
                    </button>
                    <button
                        ...
                        :class="isFullscreen ? 'bg-rose-100 text-rose-700' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'"
                    >
            ...
            <div class="hidden h-20 items-center justify-between gap-6 md:flex">
                ...
                <div class="flex min-w-fit items-center gap-3">
                    <div class="relative">
                        <div
                            class="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-rose-500 to-pink-600 shadow-lg shadow-rose-500/30 transition-transform hover:scale-105"
                        >
                    ...
                    <div>
                        <h1 class="bg-gradient-to-r from-gray-900 to-gray-600 bg-clip-text text-lg font-bold text-transparent">C19 Test 3</h1>
                    ...
                    <div class="rounded-2xl border border-gray-200 bg-white px-4 py-2.5 shadow-sm">
                        <div class="flex items-center gap-3">
                            <div class="flex items-center gap-1">
                                ...
                                <button
                                    @click="togglePlayPause"
                                    class="flex h-10 w-10 items-center justify-center rounded-full bg-rose-600 text-white shadow-md transition-all duration-200 hover:bg-rose-700 hover:shadow-lg active:scale-95"
                                >
                                    <Transition name="icon-fade" mode="out-in">
                                        <Play v-if="!isPlaying" class="ml-0.5 h-5 w-5" fill="currentColor" :key="'play'" />
                                        <Pause v-else class="h-5 w-5" fill="currentColor" :key="'pause'" />
                            ...
                            <span class="min-w-[42px] font-mono text-xs font-medium text-rose-600">{{ formatTime(currentTime) }}</span>
                ...
                <div class="flex min-w-fit items-center gap-2">
                    ...
                    >
                        ...
                        <span
                            v-if="totalNotesCount > 0"
                            class="animate-bounce-subtle absolute -top-2 -right-2 flex h-5 w-5 items-center justify-center rounded-full bg-gradient-to-br from-rose-500 to-pink-600 text-[10px] font-bold text-white shadow-lg"
                        >
                            {{ totalNotesCount }}
                        </span>
                    </button>
                    ...
                    <button
                        ...
                        :class="isFullscreen ? 'bg-rose-100 text-rose-700 hover:bg-rose-200' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'"
                    >
                    ...
                    <button
                        @click="handleSubmit"
                        class="group flex items-center gap-2 rounded-xl bg-gradient-to-r from-rose-500 to-pink-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-rose-500/30 transition-all duration-300 hover:scale-105 hover:shadow-xl active:scale-95"
                    >
    ...
    <Teleport to="body">
        <Transition name="slide">
            <div v-if="showNotesDrawer" class="fixed top-0 right-0 z-[9999] flex h-full w-full flex-col overflow-hidden bg-white shadow-2xl sm:w-96">
                <!-- Header -->
                <div class="bg-gradient-to-r from-rose-600 to-pink-600 px-4 py-4 sm:px-6">
                ...
                <div class="flex-1 overflow-y-auto px-4 py-4 sm:px-6">
                    ...
                    <div v-else class="space-y-4">
                        ...
                        >
                            ...
                            <div
                                @click="focusNote(note)"
                                class="cursor-pointer rounded border border-rose-200 bg-rose-50 px-3 py-2 transition-colors hover:bg-rose-100"
                            >
