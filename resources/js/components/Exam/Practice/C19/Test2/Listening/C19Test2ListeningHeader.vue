        ...
        <div class="container mx-auto px-3 sm:px-4 lg:px-6">
            ...
            <div class="block md:hidden">
                <div class="flex h-14 items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <h1 class="bg-gradient-to-r from-gray-900 to-gray-600 bg-clip-text text-sm font-bold text-transparent">C19 Test 2</h1>
                    </div>
                    <button
                        @click="handleSubmit"
                        class="group flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-rose-500 to-red-600 px-3 py-1.5 text-xs font-semibold text-white shadow-lg"
                    >
                ...
                <div class="mb-3 rounded-2xl border border-gray-200 bg-white p-3 shadow-sm">
                    <div class="flex items-center gap-3">
                        <button
                            @click="skipBackward"
                            class="flex h-8 w-8 items-center justify-center rounded-full text-gray-500 hover:bg-gray-100 hover:text-emerald-600"
                        >
                            <SkipBack class="h-4 w-4" />
                        </button>
                        <button
                            @click="togglePlayPause"
                            class="flex h-11 w-11 items-center justify-center rounded-full bg-emerald-600 text-white shadow-md"
                        >
                    ...
                    <div class="group mt-3">
                        <div class="relative h-1.5 cursor-pointer overflow-hidden rounded-full bg-gray-200" @click="seekByClick">
                            <div class="absolute inset-y-0 left-0 rounded-full bg-emerald-500" :style="{ width: progressPercent + '%' }"></div>
                        </div>
                    </div>
                    <div class="mt-3 flex items-center gap-2">
                        <button @click="toggleMute" class="text-gray-500 hover:text-emerald-600">
                ...
                <div class="flex items-center justify-between pb-3">
                    <button
                        ...
                        class="relative flex items-center gap-1.5 rounded-xl bg-slate-100 px-3 py-2 text-xs font-medium text-slate-700"
                    >
                        <FileText class="h-4 w-4" />
                        Notes
                        <span
                            v-if="totalNotesCount > 0"
                            class="absolute -top-1.5 -right-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-emerald-500 text-[10px] font-bold text-white"
                            >{{ totalNotesCount }}</span
                        >
                    </button>
                    <button
                        @click="toggleFullscreen"
                        class="flex items-center gap-1.5 rounded-xl bg-slate-100 px-3 py-2 text-xs font-medium text-slate-700"
                    >
            ...
            <div class="hidden h-20 items-center justify-between gap-6 md:flex">
                <div class="flex min-w-fit items-center gap-3">
                    <div class="relative">
                        <div
                            class="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 shadow-lg shadow-emerald-500/30"
                        >
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500">Cambridge 19 - Test 2 Listening</p>
                    </div>
                </div>
                    ...
                    <div class="rounded-2xl border border-gray-200 bg-white px-4 py-2.5 shadow-sm">
                        <div class="flex items-center gap-3">
                            <div class="flex items-center gap-1">
                                ...
                                <button
                                    @click="togglePlayPause"
                                    class="flex h-10 w-10 items-center justify-center rounded-full bg-emerald-600 text-white shadow-md"
                                >
                                    <Play v-if="!isPlaying" class="ml-0.5 h-5 w-5" fill="currentColor" />
                                    <Pause v-else class="h-5 w-5" fill="currentColor" />
                                </button>
                            ...
                            <div class="ml-1 flex items-center gap-2 border-l border-gray-200 pl-2">
                                ...
                                <div class="relative h-1 w-16 cursor-pointer overflow-hidden rounded-full bg-gray-200" @click="setVolumeByClick">
                                    <div
                                        class="absolute inset-y-0 left-0 rounded-full bg-emerald-500"
                                        :style="{ width: (isMuted ? 0 : volume * 100) + '%' }"
                                    ></div>
                ...
                <div class="flex min-w-fit items-center gap-2">
                    <button
                        @click="toggleNotesDrawer"
                        class="group relative flex items-center gap-2 rounded-xl bg-slate-100 px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-200"
                    >
                        <FileText class="h-4 w-4" />
                        <span class="hidden lg:block">Notes</span>
                        <span
                            v-if="totalNotesCount > 0"
                            class="absolute -top-2 -right-2 flex h-5 w-5 items-center justify-center rounded-full bg-emerald-500 text-[10px] font-bold text-white"
                            >{{ totalNotesCount }}</span
                        >
                    </button>
                    <button
                        @click="toggleFullscreen"
                        class="group flex items-center gap-2 rounded-xl bg-slate-100 px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-200"
                    >
                        <Minimize2 v-if="isFullscreen" class="h-4 w-4" />
                        <Maximize2 v-else class="h-4 w-4" />
                    ...
                    <button
                        @click="handleSubmit"
                        class="group flex items-center gap-2 rounded-xl bg-gradient-to-r from-rose-500 to-red-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg"
                    >
                        <Send class="h-4 w-4" />
            ...
            <div v-if="showNotesDrawer" class="fixed top-0 right-0 z-[9999] flex h-full w-full flex-col overflow-hidden bg-white shadow-2xl sm:w-96">
                ...
                <div class="flex-1 overflow-y-auto px-4 py-4 sm:px-6">
                    <div v-if="allNotes.length === 0" class="flex h-full flex-col items-center justify-center py-12 text-center">
                        ...
                        <p class="max-w-xs text-sm text-gray-600">Select text and click "Note" to add notes.</p>
                    </div>
                        ...
                        >
                            <div class="mb-2 flex items-start justify-between gap-2">
                                <span
                                    ...
                                    :class="{
                                        'bg-cyan-100 text-cyan-800': note.part === 'Part 1',
                                        'bg-blue-100 text-blue-800': note.part === 'Part 2',
                                        'bg-green-100 text-green-800': note.part === 'Part 3',
                                ...
                                <button
                                    ...
                                    class="flex h-6 w-6 items-center justify-center rounded-full bg-red-50 text-red-600 opacity-0 group-hover:opacity-100"
                                >
