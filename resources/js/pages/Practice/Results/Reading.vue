...
// Part-wise question ranges for Reading (40 questions, 3 parts)
...
const totalQuestions = computed(() => {
    if (isPartWise.value) {
        const range = getQuestionRange(props.partNumber!);
...
const getPartLabel = (questionNum: number) => {
    if (questionNum <= 13) return 'Part 1';
    if (questionNum <= 26) return 'Part 2';
    return 'Part 3';
};
...
const downloadPdf = async () => {
    ...
    try {
        ...
        pdf.setFontSize(22);
        pdf.setFont('helvetica', 'bold');
        pdf.text(isPartWise.value ? `Reading Test ${props.testNumber} - Part ${props.partNumber} Results` : 'Reading Test Results', margin + 35, 22);
        ...
        const filename = isPartWise.value
            ? `IELTS_Reading_Test${props.testNumber}_Part${props.partNumber}_Results.pdf`
            : `IELTS_Reading_Test${props.testNumber}_Results.pdf`;
        pdf.save(filename);
    } catch (error) {
...
<template>
    <Head :title="isPartWise ? `Reading Test ${testNumber} Part ${partNumber} Results` : `Reading Test ${testNumber} Results`" />
                ...
                >
                    <div v-if="showScore">
                        <h1 class="mb-2 text-3xl font-bold text-gray-900 sm:text-4xl">
                            Reading Test {{ testNumber }}{{ isPartWise ? ` - Part ${partNumber}` : '' }} Results
                        </h1>
            ...
            >
                <div v-if="showDetails" class="space-y-6">
                    ...
                    <div class="flex flex-col gap-4 sm:flex-row sm:justify-center">
                        <Link
                            :href="isPartWise ? `/reading/test${testNumber}/part${partNumber}` : `/exam/practice00${testNumber}/reading`"
