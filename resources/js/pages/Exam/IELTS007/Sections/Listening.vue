...
import ListeningReviewModal from '@/components/Exam/Listening/ListeningReviewModal.vue';
import ListeningHeaderIELTS007 from '@/components/Exam/ListeningIELTS007/ListeningHeaderIELTS007.vue';
import ListeningPart4IELTS007 from '@/components/Exam/ListeningIELTS007/ListeningPart4IELTS007.vue';
import ListeningPart3IELTS007 from '@/components/Exam/ListeningIELTS007/ListeningPart3IELTS007.vue';
import ListeningPart2IELTS007 from '@/components/Exam/ListeningIELTS007/ListeningPart2IELTS007.vue';
import ListeningPart1IELTS007 from '@/components/Exam/ListeningIELTS007/ListeningPart1IELTS007.vue';
import ListeningFooterIELTS007 from '@/components/Exam/ListeningIELTS007/ListeningFooterIELTS007.vue';
import ListeningSubmitModalIELTS007 from '@/components/Exam/ListeningIELTS007/ListeningSubmitModalIELTS007.vue';
...
<template>
    ...
    <div class="relative min-h-screen text-black" :class="bgColor === 'gray' ? 'bg-gray-100' : 'bg-white'">
        <ListeningHeaderIELTS007 :is-time-up="isTimeUp" :audio-volume="audioVolume" :exam-started="examStarted"
            :part1-notes="part1Notes" :part2-notes="part2Notes" :part3-notes="part3Notes" :part4-notes="part4Notes"
            @show-submit-modal="handleShowSubmitModal" @show-review-modal="handleShowReviewModal"
            @time-up="handleTimeUp" @focus-note="handleFocusNote" @delete-note="handleDeleteNote"
            @font-size-change="handleFontSizeChange" @bg-color-change="handleBgColorChange"
            @contrast-change="handleContrastChange" />
        <!-- Parts Container with Contrast Theme -->
        <div :class="[contrastClasses, 'parts-container min-h-screen transition-colors duration-300']"
            :data-theme="contrastTheme">
            <div :class="{ hidden: activePart !== 'part1' }">
                <ListeningPart1IELTS007 ref="listeningPart1Ref" :font-size="fontSize"
                    :flagged-questions="flaggedQuestions" @toggle-flag="handleToggleFlag" />
            </div>
            <div :class="{ hidden: activePart !== 'part2' }">
                <ListeningPart2IELTS007 ref="listeningPart2Ref" :font-size="fontSize"
                    :flagged-questions="flaggedQuestions" @toggle-flag="handleToggleFlag" />
            </div>
            <div :class="{ hidden: activePart !== 'part3' }">
                <ListeningPart3IELTS007 ref="listeningPart3Ref" :font-size="fontSize"
                    :flagged-questions="flaggedQuestions" @toggle-flag="handleToggleFlag" />
            </div>
            <div :class="{ hidden: activePart !== 'part4' }">
                <ListeningPart4IELTS007 ref="listeningPart4Ref" :font-size="fontSize"
                    :flagged-questions="flaggedQuestions" @toggle-flag="handleToggleFlag" />
            </div>
        </div>
        ...
        <div class="fixed right-10 bottom-40 z-40 flex gap-1">
            <button @click="goToPreviousPart" :disabled="!hasPreviousPart"
                class="flex h-10 w-11 items-center justify-center rounded transition-all duration-200" :class="hasPreviousPart
            ...
            <button @click="goToNextPart" :disabled="!hasNextPart"
        ...
        <ListeningFooterIELTS007 :active-part="activePart" :answered-questions="answeredQuestions"
            :flagged-questions="flaggedQuestions" :selected-question="selectedQuestion" :volume="audioVolume"
            @navigate="handleNavigate" @scroll-to-question="handleScrollToQuestion" @exit="handleShowSubmitModal"
            @volume-change="handleVolumeChange" />

        <!-- Review Modal -->
        <ListeningReviewModal :is-visible="showReviewModal" :user-answers="getAllUserAnswers()"
            :answered-questions="answeredQuestions" @close="handleCloseReviewModal" />

        <!-- Force Submit Modal (Reload/Back/Tab Switch Protection) -->
        <Teleport to="body">
            <div v-if="showForceSubmitModal"
        ...
        <!-- Submit Modal -->
        <ListeningSubmitModalIELTS007 :is-visible="showSubmitModal" :answered-count="answeredQuestions.size"
            :total-questions="40" :is-time-up="isTimeUp" @close="handleCancelSubmit" @submit="handleConfirmSubmit" />
        ...
        <div v-if="!examStarted"
            ...
            <audio ref="introAudioRef" src="/audio/StartListening.mpeg" preload="auto" loop />
            ...
            <div class="flex flex-col items-center justify-center px-4 text-center">
                ...
                <p class="mb-4 max-w-5xl font-bold text-sm text-white sm:text-base">
                    You will be listening to an audio clip during this test. You will not be permitted to pause or
                    rewind the audio while answering the questions.
                </p>
