...
interface Subscription {
    ...
    exam_type: string;
    remaining_full_mock_tests: number | null;
    remaining_partial_reading: number | null;
    remaining_partial_writing: number | null;
    remaining_partial_listening: number | null;
    package_name: string;
    used_full_mock_tests: number;
    used_partial_reading: number;
    used_partial_writing: number;
    used_partial_listening: number;
}
...
// Check if subscription limits allow starting this test
const canStartTest = computed(() => {
    if (!props.subscription) {
        return false;
    }

    if (props.subscription.is_expired) {
        return false;
    }

    if (props.result?.exam_type === 'full') {
        return props.subscription.remaining_full_mock_tests === null || props.subscription.remaining_full_mock_tests > 0;
    } else if (props.result?.exam_type === 'partial') {
        // For partial, check if any module still has remaining attempts
        const modules = props.result?.modules || [];
        return modules.some(module => {
            const remaining = module === 'reading' ? props.subscription!.remaining_partial_reading :
                module === 'writing' ? props.subscription!.remaining_partial_writing :
                    module === 'listening' ? props.subscription!.remaining_partial_listening :
                        0;
...
const getLimitExceededMessage = computed(() => {
    if (!props.subscription) {
    ...
    if (props.result?.exam_type === 'full' && props.subscription.remaining_full_mock_tests === 0) {
        return 'You have used all your full mock test attempts.';
    }

    return 'You have reached your test limit for one or more modules.';
});

// Confirm section info (mark as watched)
const confirmSectionInfo = (sectionName: string) => {
    if (!canStartTest.value) {
        alert(getLimitExceededMessage.value);
        return;
    }
...
<template>
    ...
    <div class="min-h-screen bg-white">
        ...
        <div class="mx-auto max-w-3xl px-4 py-6 sm:px-6 sm:py-10">
            ...
            <div v-if="subscription" class="mb-6 overflow-hidden rounded-lg border shadow-sm sm:mb-8" :class="[
                subscription.is_expired ? 'border-red-300 bg-red-50' :
                subscription.days_remaining <= 7 ? 'border-yellow-300 bg-yellow-50' :
                'border-blue-300 bg-blue-50'
            ]">
                <div class="p-4 sm:p-5">
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex-1">
                            <h3 class="text-base font-semibold sm:text-lg" :class="[
                                subscription.is_expired ? 'text-red-900' :
                                subscription.days_remaining <= 7 ? 'text-yellow-900' :
                                'text-blue-900'
                            ]">
                                Subscription Status
                            </h3>
                            <div class="mt-2 space-y-2 text-sm sm:text-base">
                                <p :class="subscription.is_expired ? 'text-red-800' : subscription.days_remaining <= 7 ? 'text-yellow-800' : 'text-blue-800'">
                                    <strong>Package:</strong> {{ subscription.package_name }}
                                </p>
                                <p v-if="!subscription.is_expired" :class="subscription.days_remaining <= 7 ? 'text-yellow-800' : 'text-blue-800'">
                                    <strong>Expires:</strong> {{ subscription.expires_at }} ({{ subscription.days_remaining }} days remaining)
                                </p>
                                <p v-else class="font-semibold text-red-800">
                    ...
                    <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                        <div v-if="subscription.exam_type === 'full'" class="rounded-lg bg-white p-3 shadow-sm">
                            <p class="text-xs font-medium text-gray-600 sm:text-sm">Full Mock Tests</p>
                            <p class="mt-1 text-lg font-bold" :class="[
                                subscription.remaining_full_mock_tests === null ? 'text-green-600' :
                                subscription.remaining_full_mock_tests > 0 ? 'text-green-600' : 'text-red-600'
                            ]">
                                {{ subscription.remaining_full_mock_tests === null ? 'Unlimited' : `${subscription.remaining_full_mock_tests} remaining` }}
                            </p>
                            <p class="mt-0.5 text-xs text-gray-500">Used: {{ subscription.used_full_mock_tests }}</p>
                        </div>
                        <template v-else>
                            <div class="rounded-lg bg-white p-3 shadow-sm">
                                <p class="text-xs font-medium text-gray-600 sm:text-sm">Reading</p>
                                <p class="mt-1 text-lg font-bold" :class="[
                                    subscription.remaining_partial_reading === null ? 'text-green-600' :
                                    subscription.remaining_partial_reading > 0 ? 'text-green-600' : 'text-red-600'
                                ]">
                                    {{ subscription.remaining_partial_reading === null ? 'Unlimited' : `${subscription.remaining_partial_reading} remaining` }}
                                </p>
                                <p class="mt-0.5 text-xs text-gray-500">Used: {{ subscription.used_partial_reading }}</p>
                            </div>
                            <div class="rounded-lg bg-white p-3 shadow-sm">
                                <p class="text-xs font-medium text-gray-600 sm:text-sm">Writing</p>
                                <p class="mt-1 text-lg font-bold" :class="[
                                    subscription.remaining_partial_writing === null ? 'text-green-600' :
                                    subscription.remaining_partial_writing > 0 ? 'text-green-600' : 'text-red-600'
                                ]">
                                    {{ subscription.remaining_partial_writing === null ? 'Unlimited' : `${subscription.remaining_partial_writing} remaining` }}
                                </p>
                                <p class="mt-0.5 text-xs text-gray-500">Used: {{ subscription.used_partial_writing }}</p>
                            </div>
                            <div class="rounded-lg bg-white p-3 shadow-sm">
                                <p class="text-xs font-medium text-gray-600 sm:text-sm">Listening</p>
                                <p class="mt-1 text-lg font-bold" :class="[
                                    subscription.remaining_partial_listening === null ? 'text-green-600' :
                                    subscription.remaining_partial_listening > 0 ? 'text-green-600' : 'text-red-600'
                                ]">
                                    {{ subscription.remaining_partial_listening === null ? 'Unlimited' : `${subscription.remaining_partial_listening} remaining` }}
                                </p>
                                <p class="mt-0.5 text-xs text-gray-500">Used: {{ subscription.used_partial_listening }}</p>
                            </div>
                        </template>
                    </div>

                    <!-- Warning if limits exceeded -->
                    <div v-if="subscription.exam_type === 'full' && subscription.remaining_full_mock_tests === 0"
                         class="mt-4 rounded-lg bg-red-100 p-3 text-sm text-red-800">
                        <strong>⚠ Limit Reached:</strong> You have used all your full mock test attempts. Please upgrade your subscription.
                    </div>
                </div>
            </div>
                        ...
                        <div v-if="!isSectionSubmitted(section.name) && !isSectionLocked(section.name)" class="mt-4">
                            ...
                            >
                                ...
                                <button
                                    ...
                                    :disabled="!hasWatchedEnough(section.name) || !canStartTest"
                                    :class="[
                                        'w-full rounded-lg px-4 py-2 text-sm font-medium transition-colors sm:py-2.5 sm:text-base',
                                        hasWatchedEnough(section.name) && canStartTest
                                            ? 'cursor-pointer bg-green-600 text-white hover:bg-green-700'
                                            : 'cursor-not-allowed bg-gray-300 text-gray-500',
                                    ]"
                                >
                                    <span v-if="!canStartTest">
                                        {{ getLimitExceededMessage }}
                                    </span>
                                    <span v-else-if="!hasWatchedEnough(section.name)">
