...
import ListeningHeaderIELTS017 from '@/components/Exam/ListeningIELTS017/ListeningHeaderIELTS017.vue';
import ListeningPart2IELTS017 from '@/components/Exam/ListeningIELTS017/ListeningPart2IELTS017.vue';
import ListeningPart3IELTS017 from '@/components/Exam/ListeningIELTS017/ListeningPart3IELTS017.vue';
import ListeningPart4IELTS017 from '@/components/Exam/ListeningIELTS017/ListeningPart4IELTS017.vue';
import ListeningFooterIELTS017 from '@/components/Exam/ListeningIELTS017/ListeningFooterIELTS017.vue';
import ListeningReviewModalIELTS017 from '@/components/Exam/ListeningIELTS017/ListeningReviewModalIELTS017.vue';
import ListeningSubmitModalIELTS017 from '@/components/Exam/ListeningIELTS017/ListeningSubmitModalIELTS017.vue';
import ListeningPart1IELTS017 from '@/components/Exam/ListeningIELTS017/ListeningPart1IELTS017.vue';
...
const correctAnswers: Record<number, string> = {
  ...
  4: 'night shift',
  5: 'swipe card',
  6: 'September 1975 / Sept 1975 / Sep 1975',
  7: 'A',
  8: 'C',
  9: 'fifteen dollars / $15 / 15 dollars / USD 15 / 15 USD',
...
const handleConfirmSubmit = async () => {
    ...
    sessionStorage.removeItem(`listening_start_time_${props.slug}`);
...
// Track visibility change debounce
let visibilityChangeTimeout: any = null;
const VISIBILITY_DEBOUNCE_MS = 300; // Debounce to avoid false positives
...
const handleVisibilityChange = () => {
    // Don't trigger if modal is already showing, if leaving page, or if exam hasn't started
    if (showForceSubmitModal.value || isLeavingPage.value || !examStarted.value) {
        return;
    }

    if (document.hidden) {
        // Use debounce to avoid false positives from browser dropdowns, autofill, etc.
        visibilityChangeTimeout = setTimeout(() => {
            // Double-check conditions after debounce
            if (document.hidden && !isLeavingPage.value && examStarted.value && !showForceSubmitModal.value) {
                showForceSubmitModal.value = true;
            }
        }, VISIBILITY_DEBOUNCE_MS);
    } else {
        // Clear timeout if visibility is restored quickly (false positive)
...
watch(examStarted, (started) => {
    if (started) {
        // Check if user has already started the exam in this session (handles reload scenario)
        // We use a timestamp-based approach to detect reloads vs fresh visits
        const examStartTime = sessionStorage.getItem(`listening_start_time_${props.slug}`);
        const currentTime = Date.now();

        if (examStartTime) {
            const startTime = parseInt(examStartTime, 10);
            // If started less than 2 hours ago, consider it the same exam session (reload)
            // This prevents false positives from old sessions
            if (currentTime - startTime < 2 * 60 * 60 * 1000) {
                // User has reloaded the page during active exam - auto submit
                showForceSubmitModal.value = true;
            } else {
                // Old session - treat as fresh start
                sessionStorage.setItem(`listening_start_time_${props.slug}`, currentTime.toString());
            }
        } else {
            // First visit - mark start time
            sessionStorage.setItem(`listening_start_time_${props.slug}`, currentTime.toString());
        }
    }
});
    ...
    <div class="relative min-h-screen text-black" :class="bgColor === 'gray' ? 'bg-gray-100' : 'bg-white'">
        ...
        <div :class="[contrastClasses, 'parts-container min-h-screen transition-colors duration-300']" :data-theme="contrastTheme">
            <div :class="{ hidden: activePart !== 'part1' }">
                <ListeningPart1IELTS017 ref="listeningPart1Ref" :font-size="fontSize" :flagged-questions="flaggedQuestions" @toggle-flag="handleToggleFlag" />

            </div>
            <div :class="{ hidden: activePart !== 'part2' }">
                <ListeningPart2IELTS017 ref="listeningPart2Ref" :font-size="fontSize" :flagged-questions="flaggedQuestions" @toggle-flag="handleToggleFlag" />
            </div>
            <div :class="{ hidden: activePart !== 'part3' }">
                <ListeningPart3IELTS017 ref="listeningPart3Ref" :font-size="fontSize" :flagged-questions="flaggedQuestions" @toggle-flag="handleToggleFlag" />
            </div>
            <div :class="{ hidden: activePart !== 'part4' }">
                <ListeningPart4IELTS017 ref="listeningPart4Ref" :font-size="fontSize" :flagged-questions="flaggedQuestions" @toggle-flag="handleToggleFlag" />
            </div>
        </div>
